...
Pain Management               |       70853|
   ...
   WHERE specialty_description IN ('Interventional Pain Management','Pain Management')
    ...
    --ans  "Interventional Pain Management" ..... '55906'
     ""      "Pain Management" ..........      "" 70853"
...
Pain Management               |       70853|
    ...
    SELECT p.specialty_description, SUM(pr.total_claim_count) AS total_claim
...
Interventional Pain Management|                |       57239|
  ...
  SELECT specialty_description,opioid_drug_flag, SUM(total_claim_count)AS total_claim
   FROM prescriber
   LEFT JOIN prescription
   USING(npi)
   LEFT JOIN drug
   USING(drug_name)
   WHERE specialty_description IN ('Interventional Pain Management','Pain Management') AND opioid_drug_flag IN('Y','N')
...
-- use ROLLUP(specialty_description, opioid_drug_flag). How does this change the result?
...
---ANS the grand total for each claim is shown
...
-- the total claim count for each of six common types of opioids: Hydrocodone, Oxycodone, Oxymorphone, Morphine, Codeine, and Fentanyl. For the purpose of this question,
-- we will put a drug into one of the six listed categories if it has the category name as part of its generic name. For example, we could count both of "ACETAMINOPHEN WITH CODEINE" and "CODEINE SULFATE" as being "CODEINE" for the purposes of this question.
...
-- Hint #3: The sql statement that goes inside of crosstab must be surrounded by single quotes. If the query that you are using also uses single quotes, you'll need to escape them by turning them into double-single quotes.
     ...
     SELECT * 
	 FROM crosstab('SELECT city,opioid_drug_flag AS drug_label,sum(total_claim_count) AS total_count
	  FROM drug
	  JOIN prescription
	  USING(drug_name)
	  JOIN 			   
			   
     WHERE drug_label IN ( "Hydrocodone"," Oxycodone", "Oxymorphone","Morphine", "Codeine","Fentanyl")
	  AND city IN ("Nashville","Memphis","Knoxville","Chattanooga")
	  ORDER BY
	  1,2',
	 'SELECT 
	  UNNEST(array["Hydrocodone"," Oxycodone", "Oxymorphone","Morphine", "Codeine","Fentanyl"])')					   
		AS(city text,
		Hydrocodone int,
	    Oxycodone int,
	    Oxymorphone int,
